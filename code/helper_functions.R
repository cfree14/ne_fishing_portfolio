...
# Format TMB output
format_output <- function(sd, stocks, covariate_name){
  ...
  results <- bind_rows(results_ln, results_theta) %>% 
    ...
    select(covariate, stockid, everything()) %>% 
    # Rename parameters
    mutate(param=recode(param, "B0"="k", "BetaT"="betaT")) %>% 
    # Gather estimate
    gather(key="val_type", value="value", 4:ncol(.)) %>% 
    # Build value name
...
format_output_re <- function(sd, stocks, dataset_name, covariate_name){
  ...
  # Estimates, wide
  est_wide <- reshape2::dcast(results.df %>% filter(param%in%params_of_interest), stockid ~ param, value.var="estimate")
  ste_wide <- reshape2::dcast(results.df %>% filter(param%in%params_of_interest), stockid ~ param, value.var="stderror")
